...
def main() -> None:
    ...
    if args.selfcheck:
        ...
        if missing:
            raise RuntimeError(f"XHS 2.7 __init__ API changed; missing={sorted(missing)} signature={sig}")
        if "fastmcp" in sys.modules:
            raise RuntimeError("XHS reduced sidecar unexpectedly imported fastmcp")
        print(f"XHS-Downloader {VERSION} reduced import/API OK; __init__={sig}; fastmcp_not_loaded=true")
        return
