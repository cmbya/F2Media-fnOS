...
- 修复 XHS-Downloader 2.7 PyInstaller `PackageNotFoundError: fastmcp-slim / fastmcp`：XHS sidecar 固定 `fastmcp==2.14.5`，显式复制 `fastmcp` distribution metadata，成品 sidecar `--selfcheck` 会再次校验 metadata。
...
- 参考 `parse-video-py 0.0.3` 接入结构化解析，优先覆盖抖音、快手、小红书、Bilibili、X；其他平台继续使用 gallery-dl / yt-dlp 探测 fallback。
