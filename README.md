...
```text
平台/用户名/YYYY-MM-DD/文件名.ext
```

例如：

```text
哔哩哔哩/UP主/2026-08-24/作品标题 P1 第一集.mp4
哔哩哔哩/UP主/2026-08-24/作品标题 P2 第二集.mp4
```

文件名以解析到的作品标题为基础：单个媒体通常保存为 `标题.ext`，图集会保存为 `标题.jpg`、`标题2.jpg` 等文件。Live Photo 会同时保存静态图片和动态视频，并使用相同的基础文件名。用户名优先使用解析结果中的用户名字段，缺少时使用作者名或作者 ID；同一平台、用户名和日期下发生文件名冲突时，文件名会自动增加 ` (2)`、` (3)` 等序号。
