...
SOURCES = {
    "yt-dlp": {"repo": "yt-dlp/yt-dlp-nightly-builds", "kind": "raw"},
    "gallery-dl": {"repo": "gdl-org/builds", "kind": "raw"},
    "x-cli": {"repo": "tamnd/x-cli", "kind": "archive", "archive_binary": "x"},
    "facebook-cli": {"repo": "tamnd/facebook-cli", "kind": "archive", "archive_binary": "fb"},
}
...
def _pick_asset(name: str, assets: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    if name == "facebook-cli":
        candidates = [x for x in assets if _is_linux_amd64_archive(str(x.get("name") or ""))]
        return next((x for x in candidates if str(x.get("name") or "").lower().startswith("fb_")), candidates[0] if candidates else None)
    return None
...
class EngineUpdater:
    ...
    async def check(self, name: str) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=20, headers={"User-Agent": "F2Media-Updater/0.2.2"}) as client:
        ...
        return {
            **local,
            ...
            "update_available": self._is_update_available(name, str(local.get("version") or ""), latest, str(local.get("release_tag") or "")),
        }
    ...
    async def update(self, name: str) -> dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(
                ...
                headers={"User-Agent": "F2Media-Updater/0.2.2"},
            ) as client:
